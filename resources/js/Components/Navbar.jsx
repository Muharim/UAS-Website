...
const Navbar = ({user}) => {
  ...
  return (
  <div className="navbar bg-white px-[16px] pt-[24px] flex justify-between">
      <a href="/">          
      ...
      {!user ?
          <ul className="hidden Laptop:flex gap-12 relative bottom-[20px] text-DGB">
      ...
      : 
            <ul className='hidden Laptop:flex gap-12 pt-[20px] relative bottom-[34px] text-DGB '>
              <li className='py-[12px] hover:text-softRed text-[18px]'><Link href={route('dashboard')}>Dashboard</Link></li>
              <li className='py-[12px] hover:text-softRed text-[18px]'><Link href={route('new')}>New</Link></li>
              <li className='py-[12px] hover:text-softRed text-[18px]'><Link href={route('popular')}>Popular</Link></li>
              <li className='py-[12px] hover:text-softRed text-[18px]'><Link href={route('logout')} method='post' as='button'>Logout</Link></li>
            </ul> }
      <button className='Laptop:hidden' onClick={toggleCanvas}><svg className='relative right-[4px] bottom-[20px]' width="40" height="17" xmlns="http://www.w3.org/2000/svg"><g fill="#00001A" fillRule="evenodd"><path d="M0 0h40v3H0zM0 7h40v3H0zM0 14h40v3H0z"/><path d="M0 0h40v3H0z"/></g></svg></button>
      {
        isCanvasOpen && <Canvas user={user} toggleCanvas={toggleCanvas}></Canvas>
      }
  </div>
  )
}
